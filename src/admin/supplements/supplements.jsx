...
export default function SupplementsAdmin() {
  ...
  const loadSupplements = async () => {
    try {
      setLoading(true);
      const data = await adminGetSupplements();
      setItems(Array.isArray(data) ? data : []);
    } catch (error) {
      toast.error(error.message || "Failed to load supplements");
    } finally {
      setLoading(false);
    }
  };
  ...
  const loadCategories = async () => {
    try {
      ...
      // Filter category list to show only nutrition/supplement categories to avoid confusion
      // e.g. Protein, Energy, Wellness, Hydration, Recovery
      const supplementCategories = ["protein", "energy", "wellness", "hydration", "recovery"];
      const filteredCats = Array.isArray(data) 
        ? data.filter(c => supplementCategories.includes(c.name.toLowerCase()))
        : [];
  ...
  const handleDelete = async () => {
    ...
    try {
      await adminDeleteSupplement(toDelete.id);
      setItems((prev) => prev.filter((p) => p.id !== toDelete.id));
      toast.success("Supplement removed successfully");
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (editingSupplement) {
        saved = await adminUpdateSupplement(editingSupplement.id, formData);
        setItems(prev => prev.map(p => p.id === saved.id ? saved : p));
        toast.success("Supplement updated successfully");
      } else {
        saved = await adminCreateSupplement(formData);
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 font-medium">Manage nutrition, vitamins, and energy inventory</p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                <th className="px-6 py-4 text-xs font-black text-slate-400 uppercase tracking-widest">Supplement</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan="5" className="px-6 py-12 text-center text-slate-400 font-medium">
                    No supplements found. Click "Add Supplement" to create one.
                  </td>
                </tr>
              ) : filtered.map((supp) => (
                <tr key={supp.id} className="hover:bg-slate-50/50 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-xl bg-slate-100 overflow-hidden flex-shrink-0 border border-slate-100 group-hover:scale-105 transition-transform">
                        {supp.imageUrl ? (
                          <img src={supp.imageUrl.startsWith('http') ? supp.imageUrl : `${IMAGE_BASE_URL}${supp.imageUrl}`} alt="" className="w-full h-full object-cover" />
                        ) : (
                          <div className="w-full h-full flex items-center justify-center text-slate-300"><FaImage /></div>
                        )}
                      </div>
                      <div className="min-w-0">
                        <p className="font-bold text-slate-900 truncate">{supp.name}</p>
                        <p className="text-xs text-slate-400 font-bold uppercase tracking-tighter">{supp.brand || "FitN Labs"}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-center">
                    <span className="font-black text-slate-900">₹{supp.price.toLocaleString()}</span>
                    {supp.discount > 0 && <p className="text-[10px] text-emerald-600 font-bold">-{supp.discount}% OFF</p>}
                  </td>
                  <td className="px-6 py-4 text-center">
                    <span className={`px-3 py-1 rounded-lg text-xs font-black shadow-inner
                      ${supp.stock > 10 ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}
                    `}>
                      {supp.stock}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-center">
                    <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-slate-100 text-slate-600 rounded-full text-xs font-bold">
                      ...
                      {categories.find(c => c.id === supp.categoryId)?.name || "Nutrition"}
                    </div>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        onClick={() => {
                          ...
                          setForm({
                            ...
                            description: supp.description || supp.shortDescription || ""
                          });
                          setImageFile(null);
                          setImagePreview(null);
                          setPanelOpen(true);
                      ...
                      <button
                        onClick={() => { setToDelete(supp); setConfirmOpen(true); }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-end p-4 md:p-6 animate-in slide-in-from-right duration-300">
          ...
          <div className="relative w-full max-w-xl bg-white h-full rounded-3xl shadow-2xl flex flex-col items-stretch overflow-hidden border border-slate-100">
            <div className="p-8 border-b border-slate-100 flex items-center justify-between">
              <div>
                <h3 className="text-2xl font-black text-slate-900">{editingSupplement ? "Update Supplement" : "New Supplement"}</h3>
                <p className="text-slate-400 text-sm font-medium">Capture details for custom coaching and elite nutrition.</p>
              </div>
            ...
            <form onSubmit={onSubmit} className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar">
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-xs font-black text-slate-500 uppercase tracking-widest pl-1">Name*</label>
                    <input
                      type="text" required
                      value={form.name}
                ...
                <div className="space-y-1">
                  ...
                  >
                    <option value="">Choose supplement category...</option>
                ...
                <div className="space-y-1">
                  <label className="text-xs font-black text-slate-500 uppercase tracking-widest pl-1">Supplement Image</label>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="relative group">
                      <input
                        type="file"
              ...
              <div className="pt-6 flex items-center gap-4">
                ...
                >
                  ...
                  <span>{editingSupplement ? "Update Supplement" : "Launch Supplement"}</span>
                </button>
              </div>
            </form>
          </div>
      ...
      {confirmOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 animate-in fade-in duration-300">
          ...
          <div className="relative max-w-sm w-full bg-white rounded-3xl shadow-2xl p-8 border border-slate-100">
            ...
            <h3 className="text-xl font-black text-slate-900 text-center">Are you absolutely sure?</h3>
            <p className="mt-2 text-slate-500 text-sm font-medium text-center">
              This will permanently remove <span className="text-slate-900 font-bold">{toDelete?.name}</span> from the database.
            </p>
